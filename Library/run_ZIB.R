...
pacman::p_load('dplyr', 'gamlss', 'pbapply', 'gamlss.dist', 'vegan')
library(devtools)
if(! require("ZIBR")) {
  library(devtools)
...
fit.ZIB <- function(features, metadata, libSize, ID, transformation, MultTestCorrection){
...
list.ZIB<-function(physeq, transformation='NONE', MultTestCorrection = 'BH'){
  foreach(physeq=physeq, .export=c("fit.ZIB"),
          .packages=c("dplyr", "gamlss", "pbapply", "gamlss.dist", "vegan", "ZIBR"), 
          .errorhandling = 'remove') %dopar% 
          {
            ...
            ID<-physeq$ID;
            DD<-fit.ZIB(features, metadata, libSize, ID, transformation, MultTestCorrection)
