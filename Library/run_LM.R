...
fit.LM <- function(features, metadata, libSize, ID, transformation, MultTestCorrection){
...
list.LM<-function(physeq, transformation='NONE', MultTestCorrection = "BH"){
  foreach(physeq=physeq, .export=c("fit.LM", "AST", "LOG", "LOGIT"),
          .packages=c("dplyr", "pbapply", "car", "nlme"), .errorhandling = 'remove') %dopar% 
  {
    ...
    ID<-physeq$ID;
    DD<-fit.LM(features, metadata, libSize, ID, transformation, MultTestCorrection)
